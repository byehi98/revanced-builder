...
- **Key Dependencies:** OpenJDK 17, `jq`, `zip`, `curl`/`wget`, `aapt2`, `apksigner`
...
- `bin/`: Contains pre-compiled binary utilities for different architectures (aapt2, htmlq, tq).
...
3. **Prebuilt Acquisition:** Fetches the latest (or specified) ReVanced CLI and patches JARs from GitHub.
4. **App Processing:** For each enabled app in `config.toml`:
    - Finds the correct APK version (from APKMirror, Uptodown, or direct).
    - Downloads the APK.
...
- **Binary Utilities:** Architecture-specific binaries are located in `bin/` and used as needed (e.g., `aapt2`, `tq`).
